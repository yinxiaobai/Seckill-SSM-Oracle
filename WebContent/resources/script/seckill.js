...
var seckill = {
		...
		countdown : function(seckillId,nowTime,startTime,endTime){
			...
			if(nowTime > endTime){
				...
				var start = eval('('+startTime+')');
				//秒杀未开始，计时事件绑定
				var killTime = new Date(start + 1000);
				//回调
				seckillBox.countdown(killTime.getTime(),function(event){
					//时间格式
				...
				if(!seckill.validatePhone(killPhone)){
					...
					$('#killPhoneBtn').click(function(){
						...
						if(seckill.validatePhone(inputPhone)){
							//电话写入cookie
							//$.cookie('killPhone',inputPhone,{expires:7,path:'/seckill'});//指定有效期(天)和路径
							//指定path之后总是获取不到cookie，原因未知；不设置path则默认为当前页面
